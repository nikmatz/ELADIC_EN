# ============================================================
# python_limits.py
# Chapter 7 — Functions, Limits and Continuity
# Matzakos, N. (2026). Elements of Linear Algebra, Differential & Integral Calculus. NewTech Publications.
# ============================================================
#
# LIBRARIES:
#   sympy      → symbolic computation of limits (limit)
#   numpy      → numerical approximation (for insight)
#   matplotlib → graphs, visualization of limits
#
# BASIC COMMANDS:
#   sympy.limit(f, x, a)           → lim_{x→a} f(x)
#   sympy.limit(f, x, a, '+')      → one-sided x→a⁺
#   sympy.limit(f, x, oo)          → lim_{x→+∞}
#   sympy.series(f, x, a, n)       → Taylor/Maclaurin expansion
#   sympy.is_continuous_at(f, a)   → continuity (by means of limit)
# ============================================================
...
print("=" * 55)
print(" Chapter 7: Functions, Limits, Continuity — Python")
print("=" * 55)

# ── A. Computing Limits (SymPy) ──────────────────────────
print("\n── A. Computing Limits ──")
...
print(f"  lp ≠ lm → lim_{{x→0}} does not exist  ✓")

print(f"  lim_{{x→0⁺}} 1/x = {limit(1/x, x, 0, '+')}")
print(f"  lim_{{x→0⁻}} 1/x = {limit(1/x, x, 0, '-')}")

# ── C. Testing Continuity ────────────────────────────────
print("\n── C. Continuity ──")

def check_continuity(f_expr, a, f_at_a, label="f"):
    """Checks whether f is continuous at a."""
...
# Discontinuous: f(0)=0 is defined but lim ≠ 0 (does not exist)
print(f"\n  h(x)=|x|/x: lim_{{x→0}} does not exist → discontinuous at 0")
...
# ── E. Taylor/Maclaurin Expansion ────────────────────────
print("\n── E. Maclaurin Expansions ──")
...
fig.suptitle("Functions, Limits and Continuity", fontsize=13, fontweight='bold')
...
ax3.set_title(r'$|x|/x$ — jump at 0', fontsize=10)
ax3.set_ylim(-2,2); ax3.grid(True,alpha=0.3); ax3.legend(fontsize=8)

# 4. Taylor sin(x) vs polynomials
...
ax5.set_title("Discontinuous at $x=1$", fontsize=10)
...
print("The figure has been saved: limits_continuity.png")
plt.show()
